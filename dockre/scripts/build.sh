...
cat <<'EOF' | $DOCKERCMD run --rm -e RUNCMD="${3:-make}" -e TERM -e HOST_UID=$(id -u) -e HOST_GID=$(id -g) -v $ABS_INPUT_PATH:/input:ro -v $ABS_OUTPUT_PATH:/output ${@:5} -w /output -i ${4} bash -x
...
cp -rau /input/. .
su -l myuser -p -c "cd /output; $RUNCMD"
BUILD_EXIT=$?
exit $BUILD_EXIT
EOF
